...
final class ChatDraftStoreTests: XCTestCase {
    func testLegacyMacDraftMigrationPreservesTextAndDoesNotResurrectConsumedDrafts() async throws {
        let suite = "LegacyDraftMigration.\(UUID().uuidString)"
        ...
        let key = ChatDraftKey.session(server: server, sessionID: "one")
        let original = "  unsent 👩🏽‍💻 text\n"
        SessionDraftPersistence.save(original, for: "one", server: server, defaults: defaults)
        defaults.set("ignore malformed key", forKey: "sessionDraft.invalid.session.invalid")
        let persistence = ChatDraftFilePersistence(directoryURL: directory, legacyDefaults: defaults)
        let imported = await persistence.load()
        XCTAssertEqual(imported, [key: ChatDraft(text: original)])
        try await persistence.write(imported)
        let restored = await persistence.load()
        XCTAssertEqual(restored[key]?.text, original)
        try await persistence.write([:])
        let consumed = await persistence.load()
        XCTAssertTrue(consumed.isEmpty)
        XCTAssertEqual(SessionDraftPersistence.load(for: "one", server: server, defaults: defaults), original)
    }
