...
struct ResponseTextSelection<Content: View>: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ controller: ResponseSelectionController, context: Context) {
        ...
        controller.input.focusScope = environment.responseSelectionFocusScope
        controller.input.onWillBeginSelection = environment.responseSelectionWillBegin
...
final class ResponseSelectionController: UIViewController {
    ...
    override func loadView() {
        ...
        input.installPointerSelection(on: host.view)
        #else
...
extension View {
    ...
    func responseSelectableText(_ text: String, separator: String = "\n", tableColumn: Int? = nil, excluding offsets: Set<Int> = []) -> some View {
        modifier(ResponseSelectionLeaf(text: text, separator: separator, tableColumn: tableColumn, excludedOffsets: offsets))
    }
}
...
private struct ResponseSelectionLeaf: ViewModifier {
    ...
    let tableColumn: Int?
    let excludedOffsets: Set<Int>
    @Environment(\.responseSelectionScope) private var scope

    @ViewBuilder
    func body(content: Content) -> some View {
        if let scope {
            content.modifier(RegisteredResponseSelectionLeaf(text: text, separator: separator, tableColumn: tableColumn, excludedOffsets: excludedOffsets, scope: scope))
        } else {
            content
        }
    }
...
private struct RegisteredResponseSelectionLeaf: ViewModifier {
    ...
    func body(content: Content) -> some View {
        let source = ResponseSelectionText(source: text, excludedOffsets: excludedOffsets)
        content
            .textRenderer(ResponseSelectionRenderer(geometry: geometry, source: source))
            .background(ResponseSelectionMarker(scope: scope, id: id, text: source.selectableText, separator: separator, tableColumn: tableColumn, geometry: geometry))
...
private struct ResponseSelectionRenderer: TextRenderer {
    ...
    let source: ResponseSelectionText
        ...
        for line in layout {
            ...
            for run in line {
                for slice in run {
                    ...
                    let renderedRange = NSRange(location: start.distance(to: lower), length: lower.distance(to: upper) + 1)
                    guard let range = source.selectableRange(for: renderedRange) else { continue }
...
/// Text.Layout glyph indices point to UTF-16 character starts, not necessarily
/// the end of an emoji/combining sequence. Expand before removing attachment
/// coordinates so neither mouse endpoints nor highlights split a character.
struct ResponseSelectionText {
    let source: String
    var excludedOffsets: Set<Int> = []

    var selectableText: String {
        guard !excludedOffsets.isEmpty else { return source }
        return String(decoding: source.utf16.enumerated().compactMap {
            excludedOffsets.contains($0.offset) ? nil : $0.element
        }, as: UTF16.self)
    }

    func selectableRange(for renderedRange: NSRange) -> NSRange? {
        let string = source as NSString
        guard renderedRange.location >= 0, renderedRange.length > 0,
              NSMaxRange(renderedRange) <= string.length else { return nil }
        let range = string.rangeOfComposedCharacterSequences(for: renderedRange)
        guard !excludedOffsets.contains(where: { NSLocationInRange($0, range) }) else { return nil }
        let removed = excludedOffsets.filter { $0 < range.location }.count
        return NSRange(location: range.location - removed, length: range.length)
    }
}

/// Per-chat focus ownership prevents the transcript's background tap handler
/// from asynchronously stealing first responder back for the composer.
final class ResponseSelectionFocusScope {
    weak var input: ResponseSelectionInput?
    var isActive: Bool { input?.isFirstResponder == true }
}

private struct ResponseSelectionFocusScopeKey: EnvironmentKey {
    static let defaultValue: ResponseSelectionFocusScope? = nil
}

private struct ResponseSelectionWillBeginKey: EnvironmentKey {
    static let defaultValue: () -> Void = {}
}

extension EnvironmentValues {
    var responseSelectionFocusScope: ResponseSelectionFocusScope? {
        get { self[ResponseSelectionFocusScopeKey.self] }
        set { self[ResponseSelectionFocusScopeKey.self] = newValue }
    }

    var responseSelectionWillBegin: () -> Void {
        get { self[ResponseSelectionWillBeginKey.self] }
        set { self[ResponseSelectionWillBeginKey.self] = newValue }
    }
}
