...
actor ChatDraftFilePersistence: ChatDraftPersisting {
    ...
    init(
        ...
        self.legacyDefaults = legacyDefaults ?? (directoryURL == nil ? .standard : nil)
    ...
    func load() async -> [ChatDraftKey: ChatDraft] {
        let legacy = legacyDefaults.map { SessionDraftPersistence.draftsForMigration(defaults: $0) } ?? [:]
        guard fileManager.fileExists(atPath: fileURL.path) else { return legacy }
        ...
        do {
            ...
            // The marker is persisted atomically with the imported drafts. Keep
            // old defaults untouched for recovery, but never resurrect them after
            // a later send or clear has persisted an empty modern store.
            let initial: [ChatDraftKey: ChatDraft] = document.didImportSessionDrafts == true ? [:] : legacy
            return (document.drafts ?? []).reduce(into: initial) { result, record in
